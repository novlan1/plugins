...
import { createLogDir, getRelativePath, parseSetDeps, saveJsonToLog } from '@plugin-light/shared';
...
import type { IChunks, IDispatchScriptOptions, IModule } from './types';
...
export class DispatchScriptPlugin {
  ...
  apply(compiler: Compiler) {
    ...
    compiler.hooks.thisCompilation.tap(this.pluginName, (compilation: Compilation) => {
      (compilation.hooks as any).optimizeChunksAdvanced.tap(this.pluginName, () => {
        try {
      ...
      compilation.hooks.optimizeChunkModules.tap(this.pluginName, (chunks, modules) => {
        ...
        try {
          this.doDispatchJS({
            modules: modules as Array<IModule>,
            chunks: chunks as IChunks,
            compilation,
          });
        } catch (err) {
          console.warn('[DISPATCH SCRIPT] err', err);
    ...
    if (this.options.addCommonVendorRequire) {
      compiler.hooks.emit.tap(this.pluginName, (compilation: Compilation) => {
        try {
  ...
  collectShouldDispatchJS(compilation: Compilation) {
    ...
    const waitDisposeModules = (compilation.modules as any).filter((module: IModule) => baseTest(module, this.options));
