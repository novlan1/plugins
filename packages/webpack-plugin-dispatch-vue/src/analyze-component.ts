...
import { getUniCliCache } from '@plugin-light/shared-vue2';
import { flattenUsingComponentMap, genIterativeComponentMap, safeJsonParse } from 't-comm';
...
export function getUsingComponentsMap(
  ...
  for (const name of jsonFileMap.keys()) {
    const jsonObj: Record<string, any> = safeJsonParse(jsonFileMap.get(name), {});
...
export function analyzeComponent(options: IDispatchVueOptions = {
  ...
  needGlobalComponents: false,
    ...
    parsedReplaceRefList?: IReplaceRefList;
  ...
  const parsedReplaceRefList: any[] = Array.from(new Set(replaceRefList.map(item => JSON.stringify(item))))
